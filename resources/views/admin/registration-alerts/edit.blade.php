...
@section('title', 'Edit Alert Registrasi')
            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0"><i class="bi bi-pencil-square me-2"></i>Edit Alert Registrasi</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.registration-alerts.update', $alert) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                                    name="title" value="{{ old('title', $alert->title) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('alert_type') is-invalid @enderror" id="alert_type"
                                    ...
                                    <option value="info" {{ old('alert_type', $alert->alert_type) == 'info' ? 'selected' : '' }}>Info (Biru)</option>
                                    <option value="warning" {{ old('alert_type', $alert->alert_type) == 'warning' ? 'selected' : '' }}>Warning (Kuning)</option>
                                    <option value="error" {{ old('alert_type', $alert->alert_type) == 'error' ? 'selected' : '' }}>Error (Merah)</option>
                                    <option value="success" {{ old('alert_type', $alert->alert_type) == 'success' ? 'selected' : '' }}>Success (Hijau)</option>
                                </select>
                                @error('alert_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-4">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active"
                                        value="1" {{ old('is_active', $alert->is_active) ? 'checked' : '' }}>
